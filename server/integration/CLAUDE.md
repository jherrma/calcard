...
| `sharing_test.go` | `TestCalendarSharing` + `TestAddressBookSharing` — owner creates a share, target sees it, permission transitions read↔read-write, revoke. Also spot-checks that an unprivileged user can't rename someone else's calendar before the share is in place. `TestSharedAddressBookRESTVisible` — the sharee's `GET /addressbooks` list carries the shared book with `shared=true` and an owner block (guards against the now-fixed "only owned" regression). `TestSharedCalendarCalDAVVisible` — the sharee can PROPFIND the owner's calendar under their own `/dav/{user}/calendars/…` path (advertised under the calendar's UUID since #47, so shares can't collide with the sharee's own paths), a sync-collection REPORT returns hrefs inside that collection, and a read-write sharee can PUT a new event that the owner then sees through REST. |
...
| `refresh_test.go` | `TestRefreshTokenFlow` — /auth/refresh rotates the refresh token (#75): a valid refresh returns a fresh access token AND a new refresh token; the OLD token is rejected the instant it is rotated; two sequential refreshes chain (token1→token2→token3); invalid / garbage tokens 401; logout revokes the current refresh token. |
| `logout_test.go` | `TestLogoutRevokesRefreshToken` — regression guard for #7: logout with the refresh token in the body revokes it server-side (a replayed token then 401s on /auth/refresh), and a logout with no body is a 200 no-op (nothing to revoke) rather than a 400. |
...
| `smoke_test.go` | `TestWellKnownRedirects` — `/.well-known/caldav` and `/.well-known/carddav` return 301 → `/dav/` (RFC 6764 discovery). `TestAuthMethods` — `GET /auth/methods` returns the local method (frontend login page calls this). `TestReadinessProbe` — `GET /ready` returns 200 (pings the DB, used by Kubernetes / load-balancer health checks). `TestUpdateProfile` — `PATCH /users/me` changes display_name and `GET /users/me` reflects it. `TestAddressBookExport` — `GET /addressbooks/:id/export` returns valid vCard containing the seeded contact. `TestAboutOpenSource` — `GET /about/open-source` 401s unauthenticated and returns the embedded attribution manifest with a token (#101); it asserts on content, not just the status, because the SPA catch-all `app.Get("/*")` would otherwise answer 200 with index.html if route ordering ever regressed. |
| `search_test.go` | `TestUnifiedSearch` — `GET /api/v1/search` (#156) as wired in `routes.go`: one query returns events, contacts, calendars and address books; an event three years ahead and one three years back are both found (no date window); a recurring series comes back once, represented by its next occurrence with a `recurrence_id`; `types=` narrows the search and marks the skipped groups `searched: false`; explicit `start`/`end` bound it when asked; a 2-character minimum and auth are enforced; and a second account sees none of it. |
| `contact_search_test.go` | `TestContactSearchCoversSharedBooks` — `GET /api/v1/contacts/search` (#162) as wired in `routes.go`: the corpus is every address book the caller can read, so a contact in a book shared with them is found (it used to vanish the moment you searched for it) while an unshared book contributes nothing; `addressbook_id` may name a shared book, and 404s — not empty-200s — for a book the caller can't read or a UUID that doesn't exist. |
| `mcp_test.go` | `TestMCPServer` — the story-104 surface over the real socket: mint a token through `POST /api/v1/mcp-tokens`, complete the `initialize` handshake (session id comes back in `Mcp-Session-Id`), `notifications/initialized` → 202, `ping`, `tools/list` (13 tools, each with an object schema), `tools/call` for `create_event` / `search_events` / `find_free_slots` / contact CRUD, `resources/list` + `resources/read`, the `GET /mcp` manifest, `DELETE /mcp` terminating the session, an unknown session → 404, another user's calendar refused, and a revoked token rejected on the very next request. The create-event step cross-checks the write through REST — the two surfaces share one use case, and that is the assertion which proves it. Requires `MCP: config.MCPConfig{Enabled: true}` in `main_test.go`; the Config there is a literal, so it does not pick up the `config.Load` default. |
...
The WebDAV auth handler (`adapter/webdav/handler.go`) resolves the Basic Auth principal via `userRepo.GetByEmail` first, falling back to `GetByUsername` (#48) — so either the email or the account username works. The URL path segment (`/dav/{username}/...`) always uses the server-assigned 16-character random username. The helpers pass the email: `registerAndLoginFull` returns the username (for URL paths), `davCall` receives the email (for Basic Auth).
