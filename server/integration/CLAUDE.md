...
| `dav_collection_test.go` | `TestDAVMkcolCalendar`, `TestDAVMkcolAddressBook`, `TestDAVDeleteAddressBook` — MKCOL creates calendars / address books via DAV (what real clients use for "+ new calendar"); DAV DELETE of an addressbook collection wipes it from the REST list too. CalDAV doesn't expose collection-delete through emersion/go-webdav so that side is not tested here. |
...
| `recurrence_test.go` | `TestRecurringEventDeleteThis`, `TestRecurringEventDeleteThisAndFuture` — scope-aware delete on recurring events. `scope=this` adds an EXDATE for the one instance; `scope=this_and_future` truncates the RRULE at the split point via UNTIL. Uses the numeric recurrence-id timestamps returned by the list endpoint, so you pass them back verbatim. |
