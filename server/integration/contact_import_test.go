...
func TestContactImportDuplicateDetection(t *testing.T) {
	...
	abID := createAddressBook(t, token, "Import Target")

	vcf := "BEGIN:VCARD\r\nVERSION:3.0\r\nUID:import-dup-1\r\nFN:Alice Example\r\nN:Example;Alice;;;\r\nEND:VCARD\r\n" +
		"BEGIN:VCARD\r\nVERSION:3.0\r\nUID:import-dup-2\r\nFN:Bob Example\r\nN:Example;Bob;;;\r\nEND:VCARD\r\n"

	// First import: both contacts are new.
	res := importContacts(t, token, abID, "skip", []byte(vcf))
	assert.Equal(t, 2, res.Imported, "first import should add both contacts")
	require.Equal(t, 2, contactCount(t, token, abID))
...
func importContacts(t *testing.T, token string, abID uint, duplicateHandling string, vcf []byte) contactImportResult {
	t.Helper()
	status, raw := rawCall(t, http.MethodPost,
		baseURL+"/api/v1/addressbooks/"+uintStr(abID)+"/import?duplicate_handling="+duplicateHandling,
...
func contactCount(t *testing.T, token string, abID uint) int {
	t.Helper()
	var resp struct {
		Total int `json:"Total"`
	}
	code := doJSONRaw(t, http.MethodGet, "/addressbooks/"+uintStr(abID)+"/contacts", token, nil, &resp)
