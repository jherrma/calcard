  ...
  <div class="space-y-6">
    ...
    <div>
      ...
      <div v-else-if="shares.length === 0" class="text-center py-8 text-surface-500">
        This address book is not shared with anyone
      </div>
...
const shares = ref<AddressBookShare[]>([]);
...
const addShare = async () => {
  ...
  try {
    await api(`/api/v1/addressbooks/${props.addressBookId}/shares`, {
      method: 'POST',
      body: {
    ...
    toast.error((e as Error).message || 'Failed to share address book');
  } finally {
    isSharing.value = false;
  }
};

const updatePermission = async (share: AddressBookShare, permission: string) => {
  try {
    await api(`/api/v1/addressbooks/${props.addressBookId}/shares/${share.id}`, {
      method: 'PATCH',
      body: { permission },
    });
...
const confirmRemove = (share: AddressBookShare) => {
  confirm.require({
    message: `Remove ${share.shared_with.display_name}'s access to this address book?`,
...
const removeShare = async (share: AddressBookShare) => {
  try {
    await api(`/api/v1/addressbooks/${props.addressBookId}/shares/${share.id}`, {
      method: 'DELETE',
    });
